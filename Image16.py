...
def try_reading_image_data(file, width, height, header_offset):
    ...
    try:
        image_data = file.read(width * height * 3 * 2)  # Multiply by 2 because we now have 16 bits per channel
        if len(image_data) < width * height * 3 * 2:
            # If the read data is too short, it's not a valid image
            return None
        image_array = np.frombuffer(image_data, dtype=np.uint16).reshape((height, width, 3))  # Change dtype to np.uint16
        return image_array
...
for filename in os.listdir(current_dir):
    if filename == "Image16.py":  # Skip the script file
        continue
    ...
    if largest_image is not None:
        image_rgb = Image.fromarray(largest_image, 'I;16')
